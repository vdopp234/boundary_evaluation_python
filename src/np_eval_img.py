...
def correspond_pixels(output_map, gt_map, max_tolerance=.01, outlier_cost=100):
    """
    Wrapper for Cython correspondPixels code
    :param output_map: Model output array
    :param gt_map: Ground truth output array (Corresponds to boundary map)
    :return: Correspondence between the two
    """
    edge_eval = EdgeEval()
    x = output_map.astype(np.double).copy(order='C')
    y = gt_map.astype(np.double).copy(order='C')
    matchE = edge_eval.getOut1(x, y)
    matchG = edge_eval.getOut2(x, y)
...
def eval_img(model_output_path, ground_truth_path, threshhold=99,
    ...
    :param threshhold: An int, specifying length of linspace
        - Used as minimum prob predicted for positive prediction 
    ...
    for k in range(K):  # Iterating through thresholds
        ...
        for g in range(n):
            ...
            match_pred = np.logical_or(match_pred, (match_pred1>0)).astype(np.double)
            match_gt = match_gt + (match_gt1 > 0).astype(np.double)
            all_gt = all_gt + ground_truth_img[g]

        cntR[k] = np.sum(match_gt)
        sumR[k] = np.sum(all_gt)
        cntP[k] = np.sum(np.logical_not(np.isclose(match_pred, np.zeros_like(match_pred))))  # Counts number of nonzero elements
        sumP[k] = np.sum(np.logical_not(np.isclose(model_output_img1, np.zeros_like(model_output_img1))))
    ...
    if save:
        # (Pycharm) Ignore this error, code is correct
